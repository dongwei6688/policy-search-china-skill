...
- **16 个部委信源覆盖** — 缓存 175+ 条政策索引，覆盖国务院、工信部、发改委、科技部、财政部等全部核心部委
...
- **政策库搜索接口降级** — gov.cn 直连被 WAF 拦截（403）时，自动经国务院政策文件库搜索接口（sousuo.www.gov.cn）获取政策原文与摘要，`--web` 还可发现缓存外的新政策；政策详情页同样支持 playwright 渲染提取全文（`fetch_gov_policy`）
