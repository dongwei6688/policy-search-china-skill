...
| 级别 | 触发条件 | 示例 |
|:----:|----------|------|
| MAJOR | 破坏性变更（架构重构、删除缓存字段、不兼容改动） | v1.9 → v2.0 |
| MINOR | 新增功能（新政策入库、新搜索模式） | v1.8 → v1.9 |
| PATCH | 修复（命名规范、格式对齐、Bug 修复） | v1.9 → v1.9.1 |

### Changelog & 发版判断矩阵

| 变更类型 | 说明 | Changelog | 版本号 | 发版 |
|:--------:|:----|:---------:|:------:|:----:|
| **数据** | | | | |
| 新政策入库 | 新增政策原文到 cache/ | ✅ 必写 | MINOR | ✅ 必发 |
| 已有政策修正 | 修正摘要/标签/来源链接 | ✅ 必写 | PATCH | ✅ 必发 |
| 撤销废止政策 | 删除已失效的政策条目 | ✅ 必写 | PATCH | ✅ 必发 |
| 缓存文件重命名 | 格式统一、命名规范对齐 | ✅ 必写 | PATCH | ✅ 必发 |
| **功能** | | | | |
| 新增搜索模式 | 如 --topic 变 --all 等新能力 | ✅ 必写 | MINOR | ✅ 必发 |
| 新增信源 | 覆盖新的部委（如新增 xxx.json） | ✅ 必写 | MINOR | ✅ 必发 |
| 跨平台适配 | 如 npx skills add、skills.sh 上架 | ✅ 必写 | MINOR | ✅ 必发 |
| Bug 修复 | 功能错误、脚本崩溃、路径问题 | ✅ 必写 | PATCH | ✅ 必发 |
| 安全修复 | 凭据泄露修复、输入校验 | ✅ 必写 | PATCH | ✅ 必发 |
| 依赖更新 | Python 包、外部工具版本升级 | ✅ 必写 | PATCH | ✅ 必发 |
| **重构** | | | | |
| 破坏性重构 | 架构变更、缓存格式不兼容 | ✅ 必写 | MAJOR | ✅ 必发 |
| 非破坏性重构 | 代码整理、性能优化 | ✅ 可选 | ❌ 不升 | ❌ 不发布 |
| **文档** | | | | |
| 纯文档改动 | README/Wiki/注释/Changelog 补漏 | ❌ 跳过 | ❌ 不升 | ❌ 不发布 |
| 文档改动含数据修正 | Changelog 补漏但 cache 也改了 | ✅ 必写 | PATCH | ✅ 必发 |
| **配置** | | | | |
| 预装脚本改动 | pre-push.sh / init.py 等 | ✅ 必写 | PATCH | ✅ 必发 |
| JSON 元信息更新 | 无数据变化，仅改字段格式 | ✅ 可选 | ❌ 不升 | ❌ 不发布 |
...
- 版本号变更必有对应 Changelog 记录，反之不成立（文档类改动仅提交不写 Changelog）
- 不升版本号的不需要发 Release，直接 `git push` 即可
- 每次 `git push` 前 pre-push hook 自动执行，版本号与 Changelog 不一致时会报警
...
| 空间 | 路径 | 特性 |
|------|------|------|
| **系统空间** | `{skill_dir}/` | 只读，更新时整体替换。包含 SKILL.md、scripts/、预装缓存(约50条) |
| **用户空间** | `~/.hermes/data/policy-search-china/` | 读写，永不覆盖。包含用户新增/修改的缓存、配置 |

**搜索优先级：用户空间 > 系统空间**

当用户空间和系统空间存在同文号（`doc_number`）的政策时，执行**一致性对比**：

```
搜索缓存时：
  1. 先查用户空间 cache/*.json
  2. 记录用户空间命中条目的 doc_number
  3. 在同信源文件中查系统空间，看是否有同 doc_number 的条目
  4. 如果两边都有（同 doc_number）→ 比较 summary + source_url + tags
     - 完全一致 → 直接返回（无冲突）
     - 不一致 → 输出对比报告，给出推荐，请用户决定
  5. 如果只在用户空间 → 返回用户空间版本
  6. 如果只在系统空间 → 返回系统空间版本
  7. 两边都没有 → 联网搜索，结果写入用户空间
```

**冲突处理原则（重要）：**

> ⚠️ **用户空间属于用户，我们只有建议权，没有处置权。**
>
> 发现两空间同文号内容不一致时：
> 1. 输出对比报告，列出两份内容的差异（如摘要差异、标签差异、来源链接差异）
> 2. 给出保留建议及理由（如"建议保留系统空间版本，因为摘要更完整/来源已验证/时效性更新"）
> 3. 询问用户意见，等待明确指令
> 4. **用户未明确说删，绝不执行任何删除用户空间内容的操作**
> 5. 用户明确要求删除后，方可操作

**合并规则（按 doc_number 去重）：**
- 系统空间更新时，`git pull` 后系统空间完整替换为用户本地新版本
- 两空间同文号的冲突在搜索时**实时对比**+**建议**，不预先合并
- 仅存在于用户空间的条目（用户自己联网搜到的）保留，不受系统空间更新影响
- 仅存在于系统空间的条目（预装但用户没搜过的）正常返回
