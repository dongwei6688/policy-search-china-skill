...
| 规则 | 原因 |
|------|------|
| **禁止手工编写 HTML 报告** | 跳过了脚本的 `source_url` 链接注入、逐字验证标签、元信息自动读取。交付物将丢失原文链接和验证标签。 |
| **所有 HTML 最终交付必须通过 `rebuild_policy_html.py` 生成** | 脚本从缓存 JSON 的 `source_url` 字段自动生成 `<a href="原文链接">`，这是唯一保证链接完整性的路径。 |
